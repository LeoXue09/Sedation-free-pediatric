...
def main():
	...
	SUV = False
	if SUV:
		save_path = '/media/data/uni/ultra_fast/data/h5_file/AC_{}_SUV'.format(acq_sec)
	else:
		save_path = '/media/data/uni/ultra_fast/data/h5_file/AC_{}'.format(acq_sec)
	...
	center_max = [2459127, 1315251*0.8, 807*0.9]
	generate_h5_3d(data_dir, acq_sec, save_path=save_path, center_max=center_max, cross_validation=cross_validation, fold=None, downsample_order=downsample_order,scale=scale,width=width,SUV=SUV)
...
def generate_h5_3d(data_dir, acq_sec, save_path, center_max, cross_validation, fold, downsample_order,scale,width,SUV):
  ...
  #                                     			shape=(0,width,width,width),expectedrows=1000000)
		...
		for pid in pid_idx:
			...
			nac = cut_pad_downsample(nac, downsample_order,scale,width)
